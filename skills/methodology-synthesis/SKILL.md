...
| **LLM-narrative daily-analysis tool** (`ZhuLinsen/daily_stock_analysis` — AI buy/hold/sell narratives + notification dashboard) | external repo, evaluated 2026-07-01 | **Unbacktestable + non-reproducible BY CONSTRUCTION:** its "strategies" are NL prompt templates ("no code required"); outputs are LLM narratives that vary per run; zero backtest/signal-rules/sizing anywhere; data is China/HK-centric (AkShare/Tushare/Longbridge). An LLM signal can't be PIT-validated (training data contains the future → lookahead) → fails `leakage-audit` + our reproducibility gate outright. Borrow ONLY as REFERENCE for a strictly-SHADOW LLM commentary layer (never the trade/signal path). Its sentiment-as-alpha angle is already REJECTED above. |
