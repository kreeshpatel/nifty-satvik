# Program state — the research board, verbatim as it stood in CLAUDE.md

**This is an archive, not the authority.** Until 2026-08-07 this text lived in `CLAUDE.md`, where it
was loaded into every session regardless of what that session was doing. It moved here so CLAUDE.md
could shrink to the rules that genuinely bind every task, because a long always-on file is one whose
instructions start getting skipped.

Nothing was cut. Where this page and a maintained source disagree, the maintained source wins:

| for | read |
|---|---|
| standing verdicts, with receipts and the cite-and-narrow rule | [`skills/program-laws`](../skills/program-laws/SKILL.md) |
| how to adjudicate a candidate without spending a trial | [`skills/verdict-machine`](../skills/verdict-machine/SKILL.md) |
| the standing counts | `diagnostics/research/n_trials.json`, `diagnostics/research/label_screen_ledger.md` |
| the pinned anchor and the plausibility bands | [`docs/references/plausibility_anchors.md`](references/plausibility_anchors.md) |
| which discipline a task is under | [`skills/session-router`](../skills/session-router/SKILL.md) |

`tests/test_standing_counts.py` includes this file among the reciters, so the counts below cannot
drift from their ledgers even though the page is an archive.
...
**screens 19 · sealed opens 2 · n_trials 2** — state them in every readout.
