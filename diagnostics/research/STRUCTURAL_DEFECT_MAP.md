...
| **4** | **D3 live ≠ certified config** | config P **fails** the 2022-26 continuous slice at **0.91** vs the **1.04** bar — a **−0.13** miss — with **−39.5% DD** (`cron:103-105`). ⚠ **CORRECTED 2026-08-09: this row previously said "vs the 1.29 bar", inflating a −0.13 miss into −0.38, roughly 3×.** 1.29 is a *different book* (all-grades + P2 + no discipline, `research/CONTEXT_ROUTER_SPEC.md:70,110`). The 1.04 bar is confirmed by three independent primary sources that pair the two numbers directly: `docs/decisions/0010-swing-config-P.md:41,50`, `research/config_CHANGELOG.md:18`, and `research/substrate/FINDING_pattern_exit.md:22,33`. The citation `cron:44-77` was also stale; the text is at `:103-105`. | TRADEOFF (owner override, recorded) | constitution D3 |
