...
def permutation_ic_pvalue(x, y, *, n_perm: int = 5000, seed: int | None = 12345) -> dict:
    """Observed Spearman IC of (x, y) plus a two-sided matched-permutation p-value: shuffle the
    pairing ``n_perm`` times and measure how often chance produces an |IC| >= |observed|.

    Returns the observed IC, the null mean/std/[5th,95th], the p-value, and the sample size. p is
    the fraction of permutations whose |IC| >= |observed| — small p => the ranking carries real,
    non-chance information."""
    ...
    if not np.isfinite(obs) or x.size < 3:
        return {"ic": obs, "n": int(x.size), "n_perm": 0, "p_value": float("nan"),
