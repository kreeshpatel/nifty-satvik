...
import { nseToday, parseCalendarDate, positionR, toTargetPct, holdWeek, demergerNotes, costBasisOf, isRebased } from '@/lib/cards';
...
function enrichSignal(raw, quotes, posBySignal) {
  ...
  // On a demerged holding the ledger re-bases this average onto the parent share before any P&L or
  // R is taken from it — the same basis the card's re-based stop is on. Without it HEG read −65% and
  // −1R against the reader's own 653.00. See costBasisOf in lib/cards.js.
  const myCost = costBasisOf(pos);
  const recordedBuy = myCost?.avg ?? null;
      ...
      <div className={`rs-tr${exiting ? ' ns-row--alarm' : ''}`}>
        ...
        <Cell v={fmtNum(s._myBuy ?? s.entry)}
              c={s.isFilledUnbooked ? 'window fill' : s._pnlIsModeled ? 'model fill'
                : isRebased(s._myCost) ? 'your fill, re-based' : 'your fill'} />
